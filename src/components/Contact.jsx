...
export default function Contact() {
  ...
  return (
    <section id="contact" className="py-28 bg-ink px-6">
      <div className="max-w-6xl mx-auto">
        <SectionLabel num="06">Contact</SectionLabel>
        <h2 ref={heading} className="reveal font-display text-4xl md:text-5xl font-bold text-cream mb-6 mt-2">Let's Talk</h2>
        <p className="text-cream-muted text-[15px] max-w-lg mb-16 leading-[1.8]">
          Open to new roles, collaborations, and conversations about data engineering.
          I typically respond within a day.
        </p>

        <div className="grid md:grid-cols-2 gap-px bg-white/3">
          {/* Contact links */}
          <div ref={linksRef} className="reveal bg-surface-2 p-10 space-y-7">
            {[
              ...
              <div key={label} className="group flex items-center gap-4">
                <div className="w-9 h-9 border border-white/6 flex items-center justify-center text-cream-faint group-hover:border-gold/40 group-hover:text-gold transition-all shrink-0">
                  <Icon size={14} />
                </div>
                <div className="min-w-0">
                  <p className="text-[9px] text-cream-faint uppercase tracking-[3px] mb-0.5">{label}</p>
                  {href ? (
                    <a href={href}
          ...
          {/* CTA card */}
          <div ref={cta} className="reveal bg-surface-3 p-10 flex flex-col justify-between border border-white/4">
            <div>
              <h3 className="font-display text-2xl font-bold text-cream mb-4">Ready to build something?</h3>
              <p className="text-cream-muted text-sm leading-[1.8]">
            ...
            <div className="mt-10 space-y-3">
              <a href={`mailto:${personal.email}`}
                 className="group flex items-center justify-between w-full bg-gold text-ink font-semibold text-sm px-6 py-4 hover:bg-gold-light transition-colors">
                Send an Email
                <HiArrowLongRight className="group-hover:translate-x-1 transition-transform" />
              </a>
              <a href={personal.linkedin} target="_blank" rel="noopener noreferrer"
                 className="flex items-center justify-between w-full border border-white/6 text-cream-muted text-sm px-6 py-4 hover:border-gold/30 hover:text-cream transition-all">
                Connect on LinkedIn
                <HiArrowLongRight size={15} />
              </a>
            </div>
          </div>
        </div>
        ...
        {/* Footer */}
        <div className="mt-20 pt-8 border-t border-white/4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <span className="text-[11px] text-cream-faint tracking-wide">© {new Date().getFullYear()} Revanth Gowda Vimala Kempegowda</span>
